...
class UserDetailsRepository(
    ...
    suspend fun getUserDetails(): UserDetails {
        // Try to load from DB
        val cached = userDatabase.load()
        if (cached != null) return cached

        // Otherwise, fetch from client in parallel
        return coroutineScope {
            val nameDeferred = async { client.getName() }
            val friendsDeferred = async { client.getFriends() }
            val profileDeferred = async { client.getProfile() }
